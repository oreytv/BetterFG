...
namespace BetterFG.UI.Tabs
{
    public class NametagColourTab : NametagWizardTab
    {
        ...
        protected override string[] StepTitles => new[] { "Text colour", "Bold / italic", "Preset style", "Ghost nametag" };
        ...
        {
            float x = PAD, cy = PAD;
            UGUIShip.CreateLabel(c, new Rect(x, cy, w, LH), "COLOUR", FS_SM, HINT);
            cy += LH + SH;
        ...
        {
            float x = PAD, cy = PAD;
            UGUIShip.CreateLabel(c, new Rect(x, cy, w, LH), "STYLE", FS_SM, HINT);
            cy += LH + SH;
            float togglew = (w - PAD * 0.5f) / 2f;
            var boldBtn = UGUIShip.CreateButton(c, new Rect(x, cy, togglew, BTN_H),
                _bold ? "Bold: ON" : "Bold: OFF", BTN_DARK2, WHITE2, FS_SM, new Action(OnToggleBold));
            ...
            var italicBtn = UGUIShip.CreateButton(c, new Rect(x + togglew + PAD * 0.5f, cy, togglew, BTN_H),
                _italic ? "Italic: ON" : "Italic: OFF", BTN_DARK2, WHITE2, FS_SM, new Action(OnToggleItalic));
        ...
        {
            float x = PAD, cy = PAD;
            UGUIShip.CreateLabel(c, new Rect(x, cy, w, LH), "PREDEFINED STYLE", FS_SM, HINT);
            cy += LH + SH;
            float stylew = (w - PAD * 1.5f) / 4f;
            float stylestep = stylew + PAD * 0.5f;
            _btnStyleNone = UGUIShip.CreateButton(c, new Rect(x, cy, stylew, BTN_H), "None",
                _nameStyle == NameStyle.None ? SEL_COLOR : BTN_DARK2, WHITE2, FS_SM, new Action(() => SetNameStyle(NameStyle.None)));
            _btnStyleDefault = UGUIShip.CreateButton(c, new Rect(x + stylestep, cy, stylew, BTN_H), "Default",
                _nameStyle == NameStyle.Default ? SEL_COLOR : BTN_DARK2, WHITE2, FS_SM, new Action(() => SetNameStyle(NameStyle.Default)));
            _btnStyleGold = UGUIShip.CreateButton(c, new Rect(x + stylestep * 2f, cy, stylew, BTN_H), "Gold",
                _nameStyle == NameStyle.Gold ? SEL_COLOR : BTN_DARK2, WHITE2, FS_SM, new Action(() => SetNameStyle(NameStyle.Gold)));
            _btnStyleGoldColored = UGUIShip.CreateButton(c, new Rect(x + stylestep * 3f, cy, stylew, BTN_H), "Gold RGB",
        ...
        {
            float x = PAD, cy = PAD;
            UGUIShip.CreateLabel(c, new Rect(x, cy, w, LH), "GHOST", FS_SM, HINT);
            cy += LH + SH;
