    ...
    public partial class SkinPackagerWindow
    {
        ...
        private void TryBuildAndPack()
        {
            ...
            try
            {
                ...
                // LZ4, never the LZMA default - LZMA fully decompresses on load and costs a ~1s hitch in game
        ...
        private GameObject ResolvePreviewObject()
        {
            ...
            // false, not true: drops the bundle handle so reopening the same file later works,
            // but keeps the prefab we just pulled out of it alive
            bundle.Unload(false);
